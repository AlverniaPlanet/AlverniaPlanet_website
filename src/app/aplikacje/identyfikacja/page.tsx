...
type Entrance = {
  id: string;
  name: string;
  rgb: string; // "R, G, B" dla rgba()
  text: string; // jasny akcent tekstu
};

// Kolory 1:1 z fizycznymi badge'ami wejść. Nazwy to marki — nie tłumaczymy ich.
const ENTRANCES: readonly Entrance[] = [
  { id: "kino360", name: "Kino 360", rgb: "247, 72, 108", text: "#ff8da3" },
  { id: "mars", name: "MARS", rgb: "247, 120, 40", text: "#ffb27a" },
  { id: "filmworld", name: "FILMWORLD", rgb: "79, 207, 222", text: "#7ef6ff" },
  { id: "vip", name: "VIP", rgb: "245, 179, 1", text: "#fcd34d" },
];
...
const TR: Record<Lang, Translation> = {
  pl: {
    ...
    subtitle2: "Wybierz swoje wejście, podaj dane i potwierdź je skanem, aby otrzymać identyfikator dostępu.",
  ...
  en: {
    ...
    subtitle2: "Choose your entrance, enter your details and confirm them with a scan to receive your access pass.",
  ...
  pt: {
    ...
    subtitle2:
      "Escolha a sua entrada, introduza os seus dados e confirme-os com uma leitura para receber o seu cartão de acesso.",
...
function EntranceButton({
  ...
  return (
    <button
      ...
      aria-pressed={active}
      onClick={onSelect}
      className={`flex items-center justify-center rounded-xl border px-2 text-center text-sm font-bold leading-tight transition hover:brightness-110 ${
        full ? "w-full flex-row gap-2 py-4" : "flex-col py-4"
      }`}
      style={{
        // Kafelki w kolorach atrakcji od razu; wybrany mocniej podświetlony.
        // VIP ma czarny środek (gold tylko na obrysie/poświacie), reszta subtelne tło w kolorze. Napisy białe.
        borderColor: `rgba(${entrance.rgb}, ${active ? 0.9 : 0.5})`,
        backgroundColor:
          entrance.id === "vip" ? "#05070e" : `rgba(${entrance.rgb}, ${active ? 0.22 : 0.1})`,
        color: "#ffffff",
        boxShadow: active ? `0 0 24px -6px rgba(${entrance.rgb}, 0.7)` : "none",
      }}
    >
      {entrance.name}
      {note ? (
        <span className={`text-[0.58rem] font-medium uppercase tracking-[0.08em] opacity-80 ${full ? "" : "mt-0.5"}`}>
          {full ? `· ${note}` : note}
        </span>
      ) : null}
    </button>
...
export default function IdentyfikacjaPage() {
  const [lang, setLang] = useState<Lang>("pl");
  const [step, setStep] = useState<Step>("form");
  const [entranceId, setEntranceId] = useState<string | null>(null);
  const [entranceMissing, setEntranceMissing] = useState(false);
  ...
  const selectedEntrance = useMemo(
    () => ENTRANCES.find((entrance) => entrance.id === entranceId) ?? null,
    [entranceId],
  );
  // Akcent całej strony = kolor wybranego wejścia (albo neutralny cyan).
  const accent = selectedEntrance ? { rgb: selectedEntrance.rgb, text: selectedEntrance.text } : NEUTRAL;
  ...
  const chooseEntrance = useCallback((id: string) => {
    setEntranceId(id);
    setEntranceMissing(false);
  }, []);
  ...
  const handleSubmit = useCallback(
    (event: React.FormEvent<HTMLFormElement>) => {
      ...
      console.log("[identyfikacja] dane formularza:", payload, "wejście:", entrance?.name);

      // Lead zapisujemy TYLKO przy zgodzie na kontakt (checkbox B). Flow identyfikacji
      // działa dalej niezależnie. Błąd logujemy (console.warn), nie pokazujemy użytkownikowi.
      if (form.consentContact) {
        void submitLead({
          ...
          entrance: entrance?.name, // np. "Kino 360", "MARS", "FILMWORLD", "VIP"
          page_url: typeof window !== "undefined" ? window.location.href : undefined,
        }).then((result) => {
          if (!result.ok) console.warn("[identyfikacja] zapis leada nieudany (pomijam):", result.error);
        });
      }
    ...
    [entranceId, form, isFormValid, cancelRaf],
  );
  ...
  const completeScan = useCallback(() => {
    ...
    setProgress(1);
    const entrance = ENTRANCES.find((e) => e.id === entranceId);
    console.log("[identyfikacja] wejście potwierdzone:", entrance?.name);
    setStep("result");
  }, [cancelRaf, entranceId]);
  ...
  return (
    <main className="relative isolate flex min-h-screen [min-height:100svh] flex-col justify-center overflow-hidden bg-[#05070e] px-4 py-8 text-white sm:px-6 sm:py-10">
      ...
      {/* Tło neutralne (cyan) — gdy brak wybranego wejścia */}
      <div
        aria-hidden="true"
        className={`pointer-events-none absolute inset-0 -z-10 transition-opacity duration-500 ${
          selectedEntrance ? "opacity-0" : "opacity-100"
        }`}
      >
      ...
      {/* Tło w kolorze wejścia — subtelne poświaty (kolor dynamiczny) */}
      <div
        aria-hidden="true"
        className={`pointer-events-none absolute inset-0 -z-10 transition-opacity duration-500 ${
          selectedEntrance ? "opacity-100" : "opacity-0"
        }`}
      >
        <div className="absolute inset-0 bg-[#05070e]" />
        <div
          className="absolute left-1/2 top-[-12%] h-[38rem] w-[38rem] -translate-x-1/2 rounded-full blur-2xl"
          style={{ background: `radial-gradient(circle, rgba(${accent.rgb},0.16), transparent 62%)` }}
        />
        <div
          className="absolute bottom-[-18%] right-[-12%] h-[32rem] w-[32rem] rounded-full blur-2xl"
          style={{ background: `radial-gradient(circle, rgba(${accent.rgb},0.12), transparent 60%)` }}
        />
      </div>
      ...
      <div ref={contentRef} className="mx-auto w-full max-w-xl [zoom:1.15] lg:max-w-2xl">
        ...
        {/* Karta — liquid glass w kolorze wejścia */}
        <div
          className="apid-in ap-glass rounded-[1.75rem] p-5 transition-colors duration-500 sm:p-7"
          style={{
            ...glassAccentStyle,
            borderColor: selectedEntrance ? `rgba(${accent.rgb}, 0.6)` : undefined,
        ...
        >
          {step === "form" ? (
            <form onSubmit={handleSubmit} noValidate className="space-y-3">
              ...
              {/* Wybór wejścia: Kino / Mars / Filmworld w rzędzie, VIP pełną szerokością pod spodem */}
              <div>
                <label className="mb-1.5 block text-sm font-medium text-white/80">{t.entranceLabel}</label>
                <div className="grid grid-cols-3 gap-2">
                  {ENTRANCES.filter((entrance) => entrance.id !== "vip").map((entrance) => (
                    <EntranceButton
                      key={entrance.id}
                      entrance={entrance}
                      active={entranceId === entrance.id}
                      onSelect={() => chooseEntrance(entrance.id)}
                    />
                  ))}
                </div>
                {ENTRANCES.filter((entrance) => entrance.id === "vip").map((entrance) => (
                ...
                {entranceMissing ? (
                  <p className="mt-1.5 text-xs font-medium text-[#ff8da3]">{t.err.entrance}</p>
                ) : null}
              </div>
          ...
          {step === "result" && selectedEntrance ? (
            <div className="flex flex-col items-center py-2 text-center">
              ...
              <p className="mt-1 text-sm text-white/55">
                {selectedEntrance.id === "vip" ? t.vipAccess : t.accessGranted}
              </p>

              {/* Zasady wizyty (jak na badge'u) */}
              <ul className="mt-5 w-full space-y-2 text-left">
                {t.visitRules.map((rule) => (
                  <li key={rule} className="flex items-start gap-2.5 text-[0.82rem] leading-snug text-white/70">
                    <span
              ...
              <button
                type="button"
                onClick={handleReset}
                className="mt-6 w-full rounded-full border border-white/20 bg-white/[0.04] px-6 py-3.5 text-sm font-semibold text-white transition hover:border-white/40 hover:bg-white/[0.1]"
              >
                {t.newId}
              </button>
            </div>
